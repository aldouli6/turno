...
function($) {
    ...
    var Notification = function(container, options) {
        ...
        function CircleNotification() {
            self.notification.addClass("pgn-circle"), self.alert.addClass("bordered");
            if((self.options.type=="warning")||(self.options.type=="danger")){
            var table = "<div data-toggle='modal' data-target='#setAjustM' onclick='manageSin("+self.options.idsin+")';>";
          }else if((self.options.type=="info")){
              var table = "<div data-toggle='modal' data-target='#showSinDetails' onclick='searchDetails("+self.options.idsin+")';>";
            }else if((self.options.type=="help")){
                var table = "<div>";
                /*data-toggle='modal' data-target='#showSinDetails' onclick='searchDetails("+self.options.idsin+")';*/
          }
            self.options.thumbnail && (table += '<div class="pgn-thumbnail"><div>' + self.options.thumbnail + "</div></div>"),
             table += '<div class="pgn-message"><div>', self.options.title && (table += '<p class="bold" style="font-size:11px !important;">' + self.options.title + "</p>"),
              table += "<p style='font-size:11px !important;font-weight:500 !important'>" + self.options.message + "</p></div></div>",
               table += "</div>";

if(self.options.type=="danger"){
                 self.options.showClose && (table += '<button type="button" class="close" data-dismiss="alert" >',
                  table += '<span aria-hidden="true";">&times;</span><span class="sr-only">Close</span>',
                   table += "</button>"),self.alert.append(table), self.alert.after('<div class="clearfix"></div>');
}else{
  self.options.showClose && (table += '<button type="button" class="close" data-dismiss="alert">',
   table += '<span aria-hidden="true">&times;</span><span class="sr-only">Close</span>',
    table += "</button>"),self.alert.append(table), self.alert.after('<div class="clearfix"></div>');
        ...
        var self = this;
        return self.container = $(container), self.notification = $('<div class="pgn"></div>'), self.options = $.extend(!0, {}, $.fn.pgNotification.defaults, options),
         self.container.find(".pgn-wrapper[data-position=" + this.options.position + "]").length ? self.wrapper = $(".pgn-wrapper[data-position=" + this.options.position + "]") : (self.wrapper = $('<div class="pgn-wrapper" data-position="' + this.options.position + '"></div>'),
