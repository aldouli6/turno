...
// ***************************************************
    ...
    $('#formEdicionUsuario').validate({ /*Método que valida que todos los campos requeridos del formulario esten llenos para poder mandar la información a la base de datos*/
        rules: {
            editTipoUsuario: {
        ...
        submitHandler: function(form) {
            ...
            // var passCiferEdit = calcMD5($("#editUsuarioPass").val());
            // $("#editUsuarioPass").val(passCiferEdit);

            // console.log($("#editUsuarioPass").val());


            if(_oldPassword != $("#editUsuarioPass").val())
            {
                var passCiferEdit = calcMD5($("#editUsuarioPass").val());
                $("#editUsuarioPass").val(passCiferEdit);                
            }
...
$('#formRegistroUsuario').validate({ /*Método que valida que todos los campos requeridos del formulario esten llenos para poder mandar la información a la base de datos*/
    rules: {
        regTipoUsuario: {
            selectcheck: true
        }
    ...
    submitHandler: function(form) 
    {
             ...
             submit este refresque la página por lo que podemos enviar nuestra información por ajax*/
              ...
              var valorTime1 = $("#horaInicio").val();
              var valorTime2 = $("#horaFin").val();
              var subfracc = 0;
              if ($('#regTipoUsuario').val() == "3" || $('#regTipoUsuario').val() == "5"){
                  subfracc = $('#SubfraccionamientoRelacion').val();
              }
              var formulario = $(form).serialize() + "&cmd=registrarUsuario&subfraccionamientoRelacion="+subfracc;
              $.ajax({
                  url: "modules/module.usuarios.php",
                  type: "POST",
                  data: formulario,
...
function eliminarUsuarioData(id_usuario) {



    swal({
    title: "\u00BFEst\u00E1s seguro de eliminar al usuario con la clave "+id_usuario+"?",
